...
import {
  MAX_SAVED_ASSIGNMENT_TABLE_VIEWS,
  ASSIGNMENT_TABLE_COLUMN_LABELS,
  ASSIGNMENT_TABLE_TOGGLEABLE_COLUMN_ORDER,
  allToggleableColumns,
  commitAssignmentTableColumnLayout,
  effectiveAssignmentToggleableColumns,
  getAssignmentFullColumnOrder,
  getAssignmentViewTriggerLabel,
  isAssignmentTableViewDirty,
  resetAssignmentTableViewToDefault,
  type AssignmentTableToggleableColumnId,
  type AssignmentTableViewsState,
} from '../lib/assignment-table-views'

export interface AssignmentTableViewPopoverProps {
  state: AssignmentTableViewsState
  effectiveToggleable: AssignmentTableToggleableColumnId[]
  onReplaceState: (next: AssignmentTableViewsState) => void
}

export function AssignmentTableViewPopover({
  state,
  effectiveToggleable,
  onReplaceState,
}: AssignmentTableViewPopoverProps) {
  const [open, setOpen] = useState(false)
  const [columnSearch, setColumnSearch] = useState('')
  const [creatingView, setCreatingView] = useState(false)
  const [newViewName, setNewViewName] = useState('')
  const [dragId, setDragId] = useState<AssignmentTableToggleableColumnId | null>(null)

  const fullOrder = useMemo(() => getAssignmentFullColumnOrder(state), [state])
  const visibleSet = useMemo(() => new Set(effectiveToggleable), [effectiveToggleable])
  const totalToggleable = ASSIGNMENT_TABLE_TOGGLEABLE_COLUMN_ORDER.length
  const visibleCount = effectiveToggleable.length
  const triggerLabel = getAssignmentViewTriggerLabel(state)
  const dirty = isAssignmentTableViewDirty(state)
  const canSaveView = dirty && state.savedViews.length < MAX_SAVED_ASSIGNMENT_TABLE_VIEWS
  ...
  const filteredOrder = useMemo(() => {
    ...
    return fullOrder.filter((id) => ASSIGNMENT_TABLE_COLUMN_LABELS[id].toLowerCase().includes(q))
  }, [columnSearch, fullOrder])

  function commit(order: AssignmentTableToggleableColumnId[], visible: AssignmentTableToggleableColumnId[]) {
    onReplaceState(commitAssignmentTableColumnLayout(state, order, visible))
  }

  function toggleColumn(id: AssignmentTableToggleableColumnId) {
  ...
  function reorderColumns(sourceId: AssignmentTableToggleableColumnId, targetId: AssignmentTableToggleableColumnId) {
  ...
  function handleReset() {
    onReplaceState(resetAssignmentTableViewToDefault(state))
    setColumnSearch('')
    setCreatingView(false)
    setNewViewName('')
  }
  ...
  function handleSaveView() {
    ...
    if (!viewName) return
    if (state.savedViews.length >= MAX_SAVED_ASSIGNMENT_TABLE_VIEWS) return
    const visible = effectiveAssignmentToggleableColumns(state)
    ...
    onReplaceState({
      ...
      savedViews: [
        ...state.savedViews,
        { id, name: viewName.slice(0, 80), columns: visible, columnOrder: order },
      ],
      activeViewId: id,
      customColumns: null,
      columnOrder: null,
        ...
        <div className="flex flex-col">
              ...
              <Button
                ...
                title={
                  !dirty
                    ...
                    : state.savedViews.length >= MAX_SAVED_ASSIGNMENT_TABLE_VIEWS
                      ? `Maximum ${MAX_SAVED_ASSIGNMENT_TABLE_VIEWS} saved views`
          ...
          <div className="px-3 py-2.5">
            ...
            <p className="mb-2 text-xs text-muted-foreground">
              License plate and Start/End date groups are always shown. Toggle and drag to customize the rest.
            </p>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      aria-label={`Reorder ${ASSIGNMENT_TABLE_COLUMN_LABELS[id]}`}
                      onMouseDown={(e) => e.stopPropagation()}
                    >
                      <GripVertical className="h-3.5 w-3.5" aria-hidden />
                    </button>
                    <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                      {ASSIGNMENT_TABLE_COLUMN_LABELS[id]}
                    </span>
                    <Switch
                      checked={checked}
                      disabled={checked && !canTurnOff}
                      onCheckedChange={() => toggleColumn(id)}
                      aria-label={`${checked ? 'Hide' : 'Show'} ${ASSIGNMENT_TABLE_COLUMN_LABELS[id]}`}
                    />
                  </li>
                )
              })}
