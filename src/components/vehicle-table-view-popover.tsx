...
import {
  MAX_SAVED_VEHICLE_TABLE_VIEWS,
  VEHICLE_TABLE_COLUMN_LABELS,
  VEHICLE_TABLE_TOGGLEABLE_COLUMN_ORDER,
  allToggleableColumns,
  commitTableColumnLayout,
...
export interface VehicleTableViewPopoverProps {
  state: VehicleTableViewsState
  effectiveToggleable: VehicleTableToggleableColumnId[]
  onReplaceState: (next: VehicleTableViewsState) => void
}

export function VehicleTableViewPopover({
  state,
  effectiveToggleable,
  onReplaceState,
}: VehicleTableViewPopoverProps) {
  const [open, setOpen] = useState(false)
  const [columnSearch, setColumnSearch] = useState('')
  const [creatingView, setCreatingView] = useState(false)
  const [newViewName, setNewViewName] = useState('')
  const [dragId, setDragId] = useState<VehicleTableToggleableColumnId | null>(null)

  const fullOrder = useMemo(() => getFullColumnOrder(state), [state])
  const visibleSet = useMemo(() => new Set(effectiveToggleable), [effectiveToggleable])
  const totalToggleable = VEHICLE_TABLE_TOGGLEABLE_COLUMN_ORDER.length
  const visibleCount = effectiveToggleable.length
  const triggerLabel = getViewTriggerLabel(state)
  const dirty = isTableViewDirty(state)
  const canSaveView = dirty && state.savedViews.length < MAX_SAVED_VEHICLE_TABLE_VIEWS
  ...
  const filteredOrder = useMemo(() => {
    ...
    return fullOrder.filter((id) => VEHICLE_TABLE_COLUMN_LABELS[id].toLowerCase().includes(q))
  }, [columnSearch, fullOrder])

  function commit(order: VehicleTableToggleableColumnId[], visible: VehicleTableToggleableColumnId[]) {
    onReplaceState(commitTableColumnLayout(state, order, visible))
  }

  function toggleColumn(id: VehicleTableToggleableColumnId) {
  ...
  function reorderColumns(sourceId: VehicleTableToggleableColumnId, targetId: VehicleTableToggleableColumnId) {
  ...
  function handleReset() {
    onReplaceState(resetTableViewToDefault(state))
    setColumnSearch('')
    setCreatingView(false)
    setNewViewName('')
  }
  ...
  function handleSaveView() {
    ...
    if (!name) return
    if (state.savedViews.length >= MAX_SAVED_VEHICLE_TABLE_VIEWS) return
    const visible = effectiveToggleableColumns(state)
        ...
        <div className="flex flex-col">
              ...
              <Button
                ...
                title={
                  !dirty
                    ...
                    : state.savedViews.length >= MAX_SAVED_VEHICLE_TABLE_VIEWS
                      ? `Maximum ${MAX_SAVED_VEHICLE_TABLE_VIEWS} saved views`
          ...
          <div className="px-3 py-2.5">
            ...
            <p className="mb-2 text-xs text-muted-foreground">
              Plate is always shown. Toggle and drag to customize the rest.
            </p>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      aria-label={`Reorder ${VEHICLE_TABLE_COLUMN_LABELS[id]}`}
                      onMouseDown={(e) => e.stopPropagation()}
                    >
                      <GripVertical className="h-3.5 w-3.5" aria-hidden />
                    </button>
                    <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                      {VEHICLE_TABLE_COLUMN_LABELS[id]}
                    </span>
                    <Switch
                      checked={checked}
                      disabled={checked && !canTurnOff}
                      onCheckedChange={() => toggleColumn(id)}
                      aria-label={`${checked ? 'Hide' : 'Show'} ${VEHICLE_TABLE_COLUMN_LABELS[id]}`}
                    />
                  </li>
                )
              })}
