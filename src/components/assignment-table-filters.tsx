...
import {
  ...
  type AssignmentFilterRow,
  activeAssignmentFilterCount,
  getAssignmentFilterOptions,
} from '../lib/assignment-table-filters'

const FILTER_KEYS: AssignmentFilterKey[] = ['type', 'status']

export interface AssignmentTableFiltersProps {
  rows: AssignmentFilterRow[]
  filters: AssignmentColumnFilters
  onChangeFilters: (next: AssignmentColumnFilters) => void
}
  ...
  return (
    <div className="border-b border-border py-3 last:border-b-0">
      <div className="mb-2 flex items-center justify-between gap-2">
        <p className="text-sm font-medium text-foreground">{ASSIGNMENT_FILTER_LABELS[filterKey]}</p>
      ...
      <ul className="max-h-32 space-y-1.5 overflow-y-auto">
        {options.map((opt) => (
          <li key={opt} className="flex items-center gap-2">
            ...
            />
            <Label htmlFor={`af-${filterKey}-${opt}`} className="cursor-pointer text-sm font-normal leading-snug">
              {opt}
            </Label>
          </li>
        ))}
...
export function AssignmentTableFilters({ rows, filters, onChangeFilters }: AssignmentTableFiltersProps) {
  const options = getAssignmentFilterOptions(rows)
  const count = activeAssignmentFilterCount(filters)
  const active = count > 0

  const setKey = (key: AssignmentFilterKey, values: string[]) => {
          ...
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            {active ? (
              <Button
                ...
                onClick={() => onChangeFilters(EMPTY_ASSIGNMENT_COLUMN_FILTERS)}
              >
                Clear all
              </Button>
            ) : null}
