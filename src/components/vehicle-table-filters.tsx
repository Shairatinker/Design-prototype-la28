...
import { cn, capitalizeFirstLetter } from './ui/utils'
import {
  EMPTY_VEHICLE_COLUMN_FILTERS,
  VEHICLE_FILTER_LABELS,
...
const FILTER_KEYS: VehicleFilterKey[] = ['type', 'status', 'currentUse', 'depot', 'maintenance']

export interface VehicleTableFiltersProps {
  rows: VehicleFilterRow[]
  filters: VehicleColumnFilters
  onChangeFilters: (next: VehicleColumnFilters) => void
}
  ...
  return (
    <div className="border-b border-border py-3 last:border-b-0">
      <div className="mb-2 flex items-center justify-between gap-2">
        <p className="text-sm font-medium text-foreground">{VEHICLE_FILTER_LABELS[filterKey]}</p>
      ...
      <ul className="max-h-32 space-y-1.5 overflow-y-auto">
        {options.map((opt) => (
          <li key={opt} className="flex items-center gap-2">
            ...
            />
            <Label htmlFor={`vf-${filterKey}-${opt}`} className="cursor-pointer text-sm font-normal leading-snug">
              {filterKey === 'maintenance' ? capitalizeFirstLetter(opt) : opt}
            </Label>
          </li>
        ))}
      </ul>
...
export function VehicleTableFilters({ rows, filters, onChangeFilters }: VehicleTableFiltersProps) {
  const options = getFilterOptions(rows)
  const count = activeFilterCount(filters)
  const active = count > 0

  const setKey = (key: VehicleFilterKey, values: string[]) => {
          ...
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            {active ? (
              <Button
                ...
                onClick={() => onChangeFilters(EMPTY_VEHICLE_COLUMN_FILTERS)}
              >
                Clear all
              </Button>
            ) : null}
