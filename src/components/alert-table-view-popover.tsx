...
import {
  ALERT_TABLE_COLUMN_LABELS,
  ALERT_TABLE_TOGGLEABLE_COLUMN_ORDER,
  MAX_SAVED_ALERT_TABLE_VIEWS,
  allAlertToggleableColumns,
  commitAlertColumnLayout,
  effectiveAlertToggleableColumns,
  getAlertFullColumnOrder,
  getAlertViewTriggerLabel,
  isAlertTableViewDirty,
  resetAlertTableViewToDefault,
  type AlertTableToggleableColumnId,
  type AlertTableViewsState,
} from '../lib/alert-table-views'

interface AlertTableViewPopoverProps {
  state: AlertTableViewsState
  effectiveToggleable: AlertTableToggleableColumnId[]
  onReplaceState: (next: AlertTableViewsState) => void
}

export function AlertTableViewPopover({
  state,
  effectiveToggleable,
  onReplaceState,
}: AlertTableViewPopoverProps) {
  const [open, setOpen] = useState(false)
  const [columnSearch, setColumnSearch] = useState('')
  const [creatingView, setCreatingView] = useState(false)
  const [newViewName, setNewViewName] = useState('')
  const [dragId, setDragId] = useState<AlertTableToggleableColumnId | null>(null)

  const fullOrder = useMemo(() => getAlertFullColumnOrder(state), [state])
  const visibleSet = useMemo(() => new Set(effectiveToggleable), [effectiveToggleable])
  const totalToggleable = ALERT_TABLE_TOGGLEABLE_COLUMN_ORDER.length
  const visibleCount = effectiveToggleable.length
  const triggerLabel = getAlertViewTriggerLabel(state)
  const dirty = isAlertTableViewDirty(state)
  const canSaveView = dirty && state.savedViews.length < MAX_SAVED_ALERT_TABLE_VIEWS
  const isAllColumnsActive = !state.activeViewId && state.customColumns === null && state.columnOrder === null

  useEffect(() => {
    if (!dirty) { setCreatingView(false); setNewViewName('') }
  }, [dirty])

  const filteredOrder = useMemo(() => {
    const q = columnSearch.trim().toLowerCase()
    if (!q) return fullOrder
    return fullOrder.filter((id) => ALERT_TABLE_COLUMN_LABELS[id].toLowerCase().includes(q))
  }, [columnSearch, fullOrder])

  function commit(order: AlertTableToggleableColumnId[], visible: AlertTableToggleableColumnId[]) {
    onReplaceState(commitAlertColumnLayout(state, order, visible))
  }

  function toggleColumn(id: AlertTableToggleableColumnId) {
    ...
    commit(fullOrder, fullOrder.filter((colId) => nextVisible.has(colId)))
  }

  function reorderColumns(sourceId: AlertTableToggleableColumnId, targetId: AlertTableToggleableColumnId) {
  ...
  function handleReset() {
    onReplaceState(resetAlertTableViewToDefault(state))
    setColumnSearch('')
  ...
  function selectAllColumnsView() {
    onReplaceState({ ...state, activeViewId: null, customColumns: null, columnOrder: null })
  }

  function selectSavedView(viewId: string) {
    onReplaceState({ ...state, activeViewId: viewId, customColumns: null, columnOrder: null })
  }

  function handleSaveView() {
    if (!dirty) return
    const name = newViewName.trim()
    if (!name || state.savedViews.length >= MAX_SAVED_ALERT_TABLE_VIEWS) return
    const visible = effectiveAlertToggleableColumns(state)
    const order = getAlertFullColumnOrder(state)
    if (visible.length === 0) return
    const id = crypto.randomUUID()
    onReplaceState({
      ...state,
      savedViews: [...state.savedViews, { id, name: name.slice(0, 80), columns: visible, columnOrder: order }],
      activeViewId: id,
      customColumns: null,
      columnOrder: null,
    })
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          className={cn(
            'h-9 min-w-[10rem] max-w-sm justify-between gap-2 rounded-lg border-2 border-border bg-card px-3 text-sm font-medium shadow-sm',
        ...
        <div className="flex flex-col">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                className={cn(
                  ...
                  canSaveView ? 'text-primary hover:bg-primary/10 hover:text-primary' : 'text-muted-foreground',
                )}
                onClick={() => { if (canSaveView) setCreatingView(true) }}
                disabled={!canSaveView}
                title={
                  !dirty
                    ? 'Customize columns to save a view'
                    : state.savedViews.length >= MAX_SAVED_ALERT_TABLE_VIEWS
                      ? `Maximum ${MAX_SAVED_ALERT_TABLE_VIEWS} saved views`
              ...
              <li>
                ...
                >
                  ...
                  <span className="shrink-0 text-xs text-muted-foreground tabular-nums">
                    {allAlertToggleableColumns().length}
                  </span>
                </button>
              </li>
          ...
          <div className="px-3 py-2.5">
            ...
            <p className="mb-2 text-xs text-muted-foreground">
              Triggered is always shown. Toggle and drag to customize the rest.
            </p>
              ...
              {filteredOrder.map((id) => {
                ...
                return (
                  <li
                    ...
                    onDrop={(e) => { e.preventDefault(); if (dragId) reorderColumns(dragId, id); setDragId(null) }}
                    className={cn(
                      'flex items-center gap-1.5 rounded-md px-1 py-1 hover:bg-muted/40',
                  ...
                  >
                    <button
                      ...
                      aria-label={`Reorder ${ALERT_TABLE_COLUMN_LABELS[id]}`}
                      onMouseDown={(e) => e.stopPropagation()}
                    >
                      <GripVertical className="h-3.5 w-3.5" aria-hidden />
                    </button>
                    <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                      {ALERT_TABLE_COLUMN_LABELS[id]}
                    </span>
                    <Switch
                      checked={checked}
                      disabled={checked && visibleSet.size <= 1}
                      onCheckedChange={() => toggleColumn(id)}
                      aria-label={`${checked ? 'Hide' : 'Show'} ${ALERT_TABLE_COLUMN_LABELS[id]}`}
                    />
                  </li>
                )
              })}
